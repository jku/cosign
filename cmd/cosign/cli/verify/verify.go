...
func PrintVerification(ctx context.Context, verified []oci.Signature, output string) {
	...
	case "text":
		for _, sig := range verified {
			...
			ui.Infof(ctx, "sig.payload() ok, stringifying...")
			_ = string(p)
			ui.Infof(ctx, "skipping printing payload")
			//fmt.Println(s)
		}

	default:
		ui.Infof(ctx, "PrintVerification output=default...")
	...
	ui.Infof(ctx, "PrintVerification ok")
}
